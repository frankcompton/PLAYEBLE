...
Команда пушит текущую ветку и печатает URL деплоя. Если новых коммитов нет, она вручную запускает workflow через GitHub CLI (`gh auth login`) или через переменную окружения `GH_TOKEN`/`GITHUB_TOKEN` с правом запускать Actions.
